...
lotr_data_test|>
  ...
  mutate(across(.cols = 1:8,
                .fns = \(x) na_if(x, ""))) -> lotr_data_test #repalcing empty cells with NA across entire tibble
```
...
```{r}
calculate_age <- function(birth_year, birth_age, death_year, death_age) { #function that caluclates the years alive of a character
...
for (x in 1:nrow(lotr_data_test)){#for loop to loop through the rows
  ...
  years_alive[x] <- calculate_age(birth_year, birth_age, death_year, death_age)}#running values through function that caluclates years alive
```
...
```{r}
lotr_data_test|>
  mutate(simplified_height_ft = (simplified_height_ft * 12),
         simplified_height_total = simplified_height_ft + simplified_height_in)|>
  select(height, simplified_height_ft, simplified_height_in, simplified_height_total)|>
  arrange(height)
```


```{r}
unique(lotr_data_test2$height)
```
